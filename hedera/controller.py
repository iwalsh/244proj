# controller.py
#
# Mininet controller for the Hedera fat-tree data center topology.
#
# This is based heavily off riplpox.py from Brandon Heller's RiplPOX project,
...
from hedera.mn import topos

from util import build_topo, get_routing
...
class HederaController(EventMixin):
    ...
    def dpid_port_to_host_index(self, dpid, port):
        ...
        return node.pod * ((self.t.k ** 2) / 4) + node.sw * (self.t.k / 2) + \
            ((port - 2) / 2)
    ...
    def _install_hybrid_dynamic_flows(self, event, out_dpid, final_out_port, packet):
        ...
        if len(src_dst_route) == 1:
            ...
            self.switches[event.dpid].install(final_out_port, match,
            ...
            self.switches[event.dpid].install_multiple(actions, match,
                                                       idle_timeout=HYBRID_IDLE_TIMEOUT,
    ...
    def _handle_packet_hybrid(self, event):
        ...
        if packet.dst in self.macTable:
            out_dpid, out_port = self.macTable[packet.dst]
            log.info("found %s on dpid %s, port %s" %
                     (packet.dst, out_dpid, out_port))
            self._install_hybrid_dynamic_flows(event, out_dpid, out_port,
                                               packet)
    ...
    def _install_hybrid_static_flows(self):
        ...
        for host in hosts:
            ...
            for core_sw in core_sws:
                ...
                k = self.t.k
                host_index = host_id.pod * k + (host_id.sw * k / 2) +\
                    (host_id.host - 2)
...
def launch(topo='ft', routing=None, mode=None):
    """
    ...
    if not topo:
        ...
        t = build_topo(topo, topos)
        r = get_routing(routing, t)

    core.registerNew(HederaController, t, r, mode)

    log.info("Hedera controller running.")

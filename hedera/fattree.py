...
# fattree.py
#
# Fat-tree topology implementation for the Hedera datacenter.
#
# Based on fattree.py from Brandon Heller's RipL project, available under the
# GPLv2 open-source license and hosted at
    ...
    class FatTreeNodeID(NodeID):
        ...
        def __init__(self, pod=0, sw=0, host=0, dpid=None, name=None):
            ...
            if dpid:
                ...
                pod, sw, host = [int(s[1:]) for s in name.split('_')]
                self.pod = pod
                self.sw = sw
                self.host = host
        ...
        def __str__(self):
            return "(p%i, s%i, h%i)" % (self.pod, self.sw, self.host)

        def name_str(self):
            '''Return name string'''
            return "p%i_s%i_h%i" % (self.pod, self.sw, self.host)
